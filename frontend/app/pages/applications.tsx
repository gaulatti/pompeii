...
export default function ApplicationsPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setAppTeamId(
        (current) => current || (currentTeam ? String(currentTeam.id) : ''),
      );
    } catch (cause: any) {
      setError(
        cause?.response?.status === 403
          ? 'You do not have permission to view applications in this scope.'
          : 'Pompeii could not load applications.',
      );
    } finally {
      setLoading(false);
    }
    ...
    },
    {
      ...
      cell: (app) => (
        <span className="app-secondary-copy text-xs text-text-secondary">
          {app.cognito_user_pool_id || 'Not configured'}
        </span>
      ),
    },
    {
      key: 'cognito_client_id',
      header: 'Cognito client',
      sortable: true,
      cell: (app) => (
        <span className="app-secondary-copy text-xs text-text-secondary">
          {app.cognito_client_id || 'Not configured'}
        </span>
      ),
    },
    {
      ...
      sortable: true,
      cell: (app) =>
        teams.find((team) => team.id === app.team_id)?.name ??
        `Team ${app.team_id}`,
    },
    {
      key: 'status',
      ...
      cell: (app) => (
        <StatusBadge
          label={
            app.cognito_user_pool_id && app.cognito_client_id
              ? 'Authenticated'
              : 'Needs Cognito IDs'
          }
          variant={
    ...
    },
    {
      ...
      cell: (app) => (
        <Button
          ...
          variant="secondary"
        >
          Open permissions <ArrowRight size={14} />
        </Button>
      ),
    },
  ];
  ...
  return (
    <div className="app-page space-y-8">
      ...
      {superAdmin && (
        <Card
          className="grid gap-6 md:grid-cols-[0.68fr_1.32fr] md:items-end"
          padding="lg"
          variant="outlined"
        >
          <div>
          ...
          <form
            ...
            onSubmit={createApp}
          >
            <Field label="Team scope" required>
              <Select
                aria-label="Team scope"
                onChange={setAppTeamId}
            ...
            >
              <Input
                required
                value={appUserPoolId}
                onChange={(event) => setAppUserPoolId(event.target.value)}
              />
            </Field>
            <Field
              className="sm:col-span-2"
      ...
      <section
        aria-labelledby="application-directory-title"
        className="space-y-5"
      >
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <SearchInput
            ...
            onClear={() => setSearch('')}
            placeholder="Search app, slug, or team"
            value={search}
          />
        </div>
        ...
        {loading ? (
          ...
          <ErrorState
            title="Application registry unavailable"
            description={error}
          ...
          <Empty
            ...
            description={
              search
                ...
                : 'Register the first application to begin defining feature policy.'
            }
            action={
              search ? (
