...
type Application = { id: number; name: string; slug: string };
...
export default function GovernancePage() {
  ...
  const loadGovernance = async () => {
    ...
    try {
      ...
      setApplicationId(
        (current) => current || String(nextApplications[0]?.id ?? ''),
      );
    } catch (cause: any) {
  ...
  const applicationRoles = roles.filter(
    (role) => !applicationId || role.application_id === Number(applicationId),
  );
  ...
  const applicationPermissions = permissions.filter(
    (permission) =>
      selectedRole && permission.application_id === selectedRole.application_id,
  );

  useEffect(() => {
    if (!applicationRoles.some((role) => role.id === selectedRoleId)) {
      setSelectedRoleId(applicationRoles[0]?.id ?? null);
    }
  }, [applicationId, roles]);
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter((user) =>
      [displayName(user), user.email].some((value) =>
  ...
  const loadAssignments = async (user: User) => {
    ...
    try {
      const data = await sendRequest(
        Method.GET,
        `authorization/users/${user.id}/role-assignments`,
      );
  ...
  const createRole = async (event: React.FormEvent) => {
    ...
    if (!applicationId || !roleName.trim() || !roleKey.trim()) return;
    setSaving(true);
    try {
      ...
      toast.error('Role could not be created', {
        description: 'Check that the role key is unique.',
      });
    } finally {
      setSaving(false);
    }
  ...
  const addAssignment = async (event: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Role assigned', {
        description:
          scope === 'global'
            ? 'Access applies globally.'
            : 'Access is limited to the selected team.',
      });
      setRoleId('');
      await loadAssignments(selectedUser);
      const logs = await sendRequest(Method.GET, 'authorization/audit-logs', {
        limit: 200,
      });
  ...
  const confirmAction = async () => {
    ...
    try {
      if (confirmState.kind === 'assignment') {
        await sendRequest(
          Method.DELETE,
          `authorization/role-assignments/${confirmState.assignment.id}`,
        );
        ...
        await sendRequest(
          Method.DELETE,
          `authorization/roles/${confirmState.role.id}`,
        );
        ...
        const updated = await sendRequest(
          Method.PATCH,
          `authorization/users/${confirmState.user.id}/active`,
          {
        ...
        setUsers((current) =>
          current.map((user) =>
            user.id === confirmState.user.id ? { ...user, ...updated } : user,
          ),
        );
    ...
    return (
      <div className="app-page space-y-8" aria-busy="true">
        <PageHeader
          ...
          title="Governance"
          description="Loading roles, assignments, and audit history…"
        />
    ...
    return (
      <div className="app-page space-y-8">
        <PageHeader
          ...
          description="Roles, assignments, and audit history could not be displayed."
        />
        <ErrorState
    ...
    <div className="app-page space-y-8">
      <PageHeader
        ...
        actions={
          <StatusBadge
            ...
            variant="live"
            description="Backend checks remain authoritative"
          />
        }
      />
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard
          title="People"
          value={users.length}
          description="Provisioned identities"
          icon={<UserCog size={17} />}
        />
        <StatCard
          title="Roles"
          value={roles.length}
          description="Global and team-ready"
        ...
        <StatCard
          ...
          description="Most recent retained view"
          icon={<Clock3 size={17} />}
        />
      </div>
        ...
        <section aria-labelledby="people-access-title" className="space-y-5">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h2 id="people-access-title" className="mt-2 text-xl font-medium">
                {filteredUsers.length} identities
              </h2>
            </div>
            <SearchInput
          ...
          {filteredUsers.length ? (
            <DataTable
              caption="People and account access"
              columns={userColumns}
              data={filteredUsers}
            ...
            <Empty
              ...
              description="Clear or refine your search to see provisioned identities."
              action={
                <Button onClick={() => setUserSearch('')} variant="secondary">
                  Clear search
                </Button>
              }
            />
        ...
        >
          <div className="space-y-5">
            <Card padding="lg" variant="surface">
              ...
              <h2 id="roles-title" className="mt-2 text-xl font-medium">
                Configurable roles
              </h2>
              <p className="app-secondary-copy mt-2 text-sm leading-6 text-text-secondary">
                Every role belongs to one application and can only contain
                permissions from that application.
              </p>
              <div className="mt-5">
                <Field label="Application" required>
                  <Select
                    aria-label="Role application"
                ...
                {applicationRoles.map((role) => (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      ...
                      <span className="app-secondary-copy block truncate text-[11px] text-text-secondary">
                        {selectedApplication?.slug} · {role.key}
                      </span>
                    </span>
                    <StatusBadge
                      label={role.is_system ? 'System' : 'Custom'}
                      variant={role.is_system ? 'info' : 'default'}
                    />
                  </button>
                ))}
              </div>
            ...
            <Card padding="lg" variant="subtle">
              ...
              <p className="app-secondary-copy mt-2 text-sm text-text-secondary">
                The role will belong to{' '}
                {selectedApplication?.name || 'the selected application'}.
              </p>
              <form className="mt-5 space-y-4" onSubmit={createRole}>
                ...
                <Field
                  label="Role key"
                  description="Use a stable identifier such as billing-reviewer."
                  required
                >
                  <Input
                    required
          ...
          <div className="space-y-5">
            <Card padding="lg" variant="elevated">
              {selectedRole ? (
                <>
                  <div className="flex flex-wrap items-start justify-between gap-4 border-b border-sand/20 pb-5 dark:border-white/[0.08]">
                    <div>
                      ...
                      <p className="app-secondary-copy mt-2 max-w-xl text-sm leading-6 text-text-secondary">
                    ...
                    {!selectedRole.is_system && (
                      <Button
                        onClick={() =>
                          setConfirmState({ kind: 'role', role: selectedRole })
                        }
                        size="sm"
                        variant="destructive"
                      >
                ...
                <Empty
                  title="Select a role"
                  description="Choose a role to review its effective permission mapping."
                />
              )}
            </Card>
            ...
            <Card padding="lg" variant="outlined">
              ...
              <p className="app-secondary-copy mt-2 text-sm leading-6 text-text-secondary">
                Register or import permission contracts from the owning
                application’s page. Governance maps those registered permissions
                to roles.
              </p>
              <Button
                as="a"
      ...
      {activeTab === 'audit' && (
        <section aria-labelledby="audit-title" className="space-y-5">
          <div>
            ...
            <h2 id="audit-title" className="mt-2 text-xl font-medium">
              Recent sensitive actions
            </h2>
            <p className="app-secondary-copy mt-2 text-sm text-text-secondary">
              The service returns the most recent 200 entries for this
              operational view.
            </p>
          </div>
          {auditLogs.length ? (
            ...
            <Empty
              ...
              description="Sensitive administrative actions will appear here as they occur."
              icon={<Clock3 size={26} />}
            />
          )}
        </section>
      ...
      <Sheet
        description={
          selectedUser
            ? `Global and team-scoped roles for ${displayName(selectedUser)}`
            : ''
        }
        ...
        {selectedUser && (
          <div className="space-y-8">
            <form className="space-y-4" onSubmit={addAssignment}>
              ...
              <Field label="Role" required>
                <Select
                  ...
                  options={roles.map((role) => ({
                    label: `${role.application?.name || applications.find((application) => application.id === role.application_id)?.name || 'Application'} · ${role.name}`,
              ...
              <Field
                label="Scope"
                description="Global applies across every team. Team scope is evaluated only for that team."
                required
              >
                <Select
            ...
            <div className="border-t border-sand/20 pt-6 dark:border-white/[0.08]">
              ...
              {assignmentsLoading ? (
                ...
                <ul className="mt-4 divide-y divide-sand/20 border-y border-sand/20 dark:divide-white/[0.08] dark:border-white/[0.08]">
                  {assignments.map((assignment) => {
                    const role =
                      assignment.role ??
                      roles.find(
                        (current) => current.id === assignment.role_id,
                      );
                    const team = teams.find(
                      (current) => current.id === assignment.team_id,
                    );
                    return (
                      <li
                      ...
                      >
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="app-secondary-copy mt-1 text-xs text-text-secondary">
                            {team
                              ? `Team · ${team.name}`
                              : 'Global · all teams'}
                          </p>
                        ...
                        <IconButton
                          aria-label={`Remove ${role?.name || 'role'} assignment`}
                          onClick={() =>
                            setConfirmState({ kind: 'assignment', assignment })
                          }
                          size="sm"
                          variant="ghost"
                        >
                          <Trash2 size={15} className="text-terracotta" />
                        </IconButton>
                      </li>
                    );
                  })}
                ...
                <Empty
                  ...
                  description="This person has no global or team-scoped RBAC roles."
                />
              )}
            </div>
          </div>
